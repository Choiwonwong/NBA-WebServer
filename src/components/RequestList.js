...
const requestListData = [
  { id: 1, repo: 'https://github.com/CHOIWONWENG/test0531.git', branch: 'main', state: '프로비저닝 중', emessage: null },
  { id: 2, repo: 'https://github.com/CHOIWONWENG/test0531.git', branch: 'Master', state: '빌드 에러', emessage: "등록 권한 부족"},
  { id: 3, repo: 'https://github.com/CHOIWONWENG/test0531.git', branch: 'blue', state: '정상 배포', emessage: null},
  // 추가 요청 데이터
];
...
function RequestList() {
  return (
    <div className="container">
      <Link to="/" className="quest-title">Quest</Link>
  ...
  <tr key={item.id} className="request-table-row">
    ...
    <Link to={`/request/${item.id}`} className="request-list-link">
                {item.state}</Link></td>
    <td>          <Link to={`/request/${item.id}`} className="request-list-link">
    {item.emessage}</Link></td>
  </tr>
))}
